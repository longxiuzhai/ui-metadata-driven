...
客户列表和订单列表分别由 `customer_list`、`order_list` 元数据页面装配。“查看”进入客户详情；客户详情中的“查看订单”进入带 `customerId` 的订单列表。

`DataTableCard` 支持元数据配置的表格工具栏、行操作和冻结列。客户、订单列表在工具栏提供“新增”，行操作列提供“查看 / 修改 / 删除”；操作列默认冻结在右侧，客户 ID、订单号冻结在左侧。业务列可在 `columns[].fixed` 中配置 `left` 或 `right`。新增、修改、删除成功后局部刷新列表；存在关联订单的客户会拒绝删除。

客户列表和订单列表的查询表单分别由各自业务 View 维护，`DataTableCard` 只展示结果。客户支持客户 ID、手机号精确查询、名称模糊查询和状态筛选；订单支持订单号、客户 ID、会员 ID 精确查询和状态筛选。从客户详情进入订单列表时，客户 ID 由页面上下文固定，不再提供跨客户查询。
